...
<form class="ymo-card mb-3" method="get" action="<?= admin_url('customers'); ?>">
...
<div class="ymo-card p-0">
    <table class="ymo-table mb-0">
        ...
        <tbody>
        <?php if (empty($rows)): ?>
            <tr><td colspan="6" class="text-center py-4 ymo-muted">No customers found.</td></tr>
        <?php endif; ?>
        <?php foreach ($rows as $u): ?>
            <tr>
                ...
                <td class="text-end"><a class="small" href="<?= admin_url('customers/'.$u['id']); ?>">Open<span class="mi mi-sm mi-trailing">arrow_forward</span></a></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <nav class="mt-3">
        ...
        <?php for ($i = 1; $i <= $pages; $i++): ?>
            <li class="page-item <?= $i === $page ? 'active' : ''; ?>">
                <a class="page-link" href="<?= admin_url('customers?page='.$i.'&q='.urlencode((string) $q)); ?>"><?= $i; ?></a>
            </li>
        <?php endfor; ?>
        </ul>
    </nav>
<?php endif; ?>
<p class="ymo-muted small mt-2"><?= (int) $total; ?> total customer<?= $total === 1 ? '' : 's'; ?>.</p>
