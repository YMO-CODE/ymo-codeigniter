...
|--------------------------------------------------------------------------
|
| Dual-host deployments: booking.yourmechaniconline.com serves customers;
| admin.yourmechaniconline.com serves the panel (short paths via routes.php).
| Set YMO_PUBLIC_APP_URL (+ optional YMO_ADMIN_APP_URL); YMO_APP_URL remains
| a backwards-compatible fallback for the public/booking hostname.
|
*/
require_once dirname(__FILE__).'/ymo_host.php';

$public_candidate = getenv('YMO_PUBLIC_APP_URL');
if ($public_candidate === FALSE || trim((string) $public_candidate) === '') {
    $public_candidate = getenv('YMO_APP_URL');
}

$cli_bootstrap = (PHP_SAPI === 'cli' || PHP_SAPI === 'phpdbg');
...
|
*/
$config['log_path'] = realpath(FCPATH.'../storage/logs') ? FCPATH.'../storage/logs/' : '';
...
|
*/
$config['encryption_key'] = getenv('YMO_ENCRYPTION_KEY') ?: '';
...
$config['sess_cookie_name'] = 'ymo_session';
...
$config['csrf_token_name'] = 'ymo_csrf_token';
